...
class Cell:
    ...
    def __add__(self, other):
        return f'Сложение - {Cell(self.quantity + other.quantity)}'

    def __sub__(self, other):

        if self.quantity - other.quantity > 0:
            return f'Вычитание - {Cell(self.quantity - other.quantity)}'
        else:
            return f'Разность отрицательна, поэтому операция не выполняется'

    def __mul__(self, other):
        return f'Умножение - {Cell(int(self.quantity * other.quantity))}'

    def __truediv__(self, other):
        return f'Деление - {Cell(round(self.quantity // other.quantity))}'
...
print(cell1 * cell2)
print(cell1 / cell2)
print(cell2.make_order(22))
# В описании задания получаемых по метод make_order() - ошибка.
# Cидел чуть меньше часа, чтобы понять, почему такие значения.
# Если правильно понял, то их и не получишь.
